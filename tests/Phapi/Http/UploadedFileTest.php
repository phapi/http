...
/**
 * @coversDefaultClass \Phapi\Http\UploadedFile
 */
class UploadedFileTest extends TestCase
{
    ...
    {
        $this->tmpFile = $stream = tempnam(sys_get_temp_dir(), 'phapi');
        $upload = new UploadedFile($stream, 0, UPLOAD_ERR_OK);
    ...
    {
        $upload = new UploadedFile(fopen('php://temp', 'wb+'), 0, UPLOAD_ERR_OK, 'foobar.baz', 'mpg');

        $this->assertEquals(0, $upload->getSize());
        $this->assertEquals(UPLOAD_ERR_OK, $upload->getError());
        $this->assertEquals('foobar.baz', $upload->getClientFilename());
        $this->assertEquals('mpg', $upload->getClientMediaType());
    }
}